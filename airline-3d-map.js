...
(function(){
  ...
  function populateRouteFilters(model){
    ...
    [source,destination].forEach((select,index)=>{
      const values=[...select.options].slice(1).map(option=>option.value);
      const parent=select.parentElement;
      parent.querySelector(".route-picker")?.remove();
      const picker=document.createElement("div");
      picker.className="route-picker";
      const trigger=document.createElement("button");
      trigger.type="button";
      trigger.className="route-picker-trigger";
      trigger.textContent=select.value||(index===0?"All sources":"All destinations");
      trigger.setAttribute("aria-label",index===0?"Source market":"Destination");
      const menu=document.createElement("div");
      menu.className="route-picker-menu";
      const search=document.createElement("input");
      search.type="search";
      search.className="route-picker-search";
      search.placeholder="Type to find an option...";
      search.setAttribute("aria-label",index===0?"Search source market":"Search destination");
      menu.appendChild(search);
      const optionList=document.createElement("div");
      optionList.className="route-picker-options";
      const allLabel=index===0?"All sources":"All destinations";
      ["",...values].forEach(value=>{
        const option=document.createElement("button");
        option.type="button";
        option.className="route-picker-option";
        option.dataset.value=value;
        option.textContent=value||allLabel;
        option.addEventListener("click",()=>{
          select.value=value;
          trigger.textContent=value||allLabel;
      ...
      select.classList.add("route-select-hidden");
      trigger.addEventListener("click",event=>{
        event.stopPropagation();
        document.querySelectorAll(".route-picker-menu.open").forEach(other=>{if(other!==menu)other.classList.remove("open");});
        menu.classList.toggle("open");
        if(menu.classList.contains("open")){search.value="";search.focus();}
      });
      search.addEventListener("input",()=>{
        const query=search.value.trim().toLowerCase();
        optionList.querySelectorAll(".route-picker-option").forEach(option=>{
          option.style.display=!query||option.textContent.toLowerCase().includes(query)?"block":"none";
        });
      });
      select.addEventListener("change",()=>{trigger.textContent=select.value||allLabel;});
    });
  ...
  function bindControls(){
    ...
    if(clear)clear.addEventListener("click",()=>{routeSource="";routeDestination="";selectedDetail=null;if(source){source.value="";source.parentElement.querySelector(".route-picker-trigger").textContent="All sources";}if(destination){destination.value="";destination.parentElement.querySelector(".route-picker-trigger").textContent="All destinations";}renderAirline3DMap(true);});
